...
function start(){
		...
		pageUrls.forEach(function(pageUrl){
			superagent.get(pageUrl)
				...
				.end(function(err,pres){
					...
					var liArr = $('.lnews').children().first().children();
					liArr.map(function(i, li){
						var strTitle = $(this).find('a').first().attr('title')
						console.log('Title ' + strTitle);
						// var reg = /[成都房产成交量]*住宅备案成交/;
						var reg = /住宅备案成交([0-9]+)套/;
						var res = reg.exec(strTitle);
						console.log('Regular result: ' + res);
						if (res != null) {
							var intQuantity = res[1];
							console.log('Title ' + strTitle + ' Quantity ' + intQuantity);
	...
	http.createServer(onRequest).listen(3007);
}
